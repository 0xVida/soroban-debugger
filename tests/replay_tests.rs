...
fn create_sample_trace(contract_path: &str) -> NamedTempFile {
    ...
    fs::write(trace_file.path(), serde_json::to_string_pretty(&trace_json).unwrap()).unwrap();
    trace_file
}
...
fn test_replay_with_sample_trace_files() {
    ...
    assert!(
        stdout.contains("Loading trace file") || stderr.contains("Loading trace file") ||
        stdout.contains("Failed to read") || stderr.contains("Failed to read"),
...
fn test_replay_until_flag() {
    ...
    assert!(
        stdout.contains("Loading trace") || stderr.contains("Loading trace") ||
        stdout.contains("Replaying up to step") || stderr.contains("Replaying up to step") ||
        stdout.contains("Failed to read") || stderr.contains("Failed to read"),
...
fn test_replay_contract_override() {
    ...
    assert!(
        stdout.contains("new_contract.wasm") || stderr.contains("new_contract.wasm") ||
        stdout.contains("Loading contract") || stderr.contains("Loading contract"),
